...
export default class Controller extends Command {
    ...
    static flags = {
        ...
        "financial-protocol": CustomFlags.address({
            description:
                "Address of the contract to watch for application deployments",
            env: "SUNODO_FINANCIAL_PROTOCOL",
        }),
        "machine-protocol": CustomFlags.address({
            description:
                "Address of the contract to watch for application machine locations",
            env: "SUNODO_MACHINE_PROTOCOL",
        }),
        factory: CustomFlags.address({
            description:
                "Address of the PayableDAppFactory contract to watch for application deployments",
            env: "SUNODO_FACTORY",
        }),
    ...
    public async run(): Promise<void> {
        ...
        const machineProtocol = flags.factory || flags["machine-protocol"];
        if (!machineProtocol) {
        ...
        const machineContract = getContract({
            ...
            address: machineProtocol,
            publicClient,
        });
        const financialContract = getContract({
            abi: iFinancialProtocolABI,
            address: financialProtocol,
            publicClient,
        });

        // get logs I missed since last execution
        this.log(
            `listening to contract ${machineProtocol} from block ${fromBlock} for machine location events`,
        );
        this.log(
            `listening to contract ${financialProtocol} from block ${fromBlock} for financial runway events`,
        );
