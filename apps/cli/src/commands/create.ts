...
export default class CreateCommand extends Command {
    ...
    private async download(
        ...
        const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'sunodo-create-template-'));
        ...
        try {
            ...
            await execa("docker", ["container", "cp", `${cid}:/${template}/`, out], { stdio: "inherit" });
        } finally {
            // cleanup
            await execa("docker", ["container", "rm", cid], { stdio: "inherit" });
            await execa("docker", ["image", "rm", dockerName], { stdio: "inherit" });
