    ...
    IMachineProtocol
{
    ...
    function register(ICartesiDApp dapp, string memory location) external {
        emit MachineLocation(address(dapp), location);
    }
}
    ...
    IValidatorNodeProvider
{
    ...
    function deploy(
        ...
        emit MachineLocation(address(dapp), _location);

        // set initial runway
        _extendRunway(msg.sender, dapp, _initialRunway);
